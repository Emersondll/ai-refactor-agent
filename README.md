...
- **N7 — P4 detecta toString de objeto**: `_try_surgical_patch` case 1 (expected=null) tinha bug em que sempre quotava `actual` como String literal. Quando `actual` era toString de objeto (ex: `TransactionCodeModel[code=null]`), produzia `assertEquals("...", expr)` que compila mas falha pior em runtime com type-mismatch. Fix: detecta `[/]/=/(/)` no actual e usa `assertNotNull(expr)` em vez de quotar (única afirmação segura — sabemos não-null mas não podemos afirmar valor)
- **N8 — Revert do patch P4 quando piorou o erro**: `_maven_output_got_worse(old_out, new_out)` compara assinaturas normalizadas (sem paths/linhas/valores) de `[ERROR]` lines. Retorna True se nova saída tem MAIS errors ou nova categoria. Safety net pós-P4 no repair loop: salva `_original_code`/`_original_out`, aplica patch, se Maven volta a falhar e ficou pior, REVERTE `write_file` + variáveis antes de cair no LLM repair. Garante que o LLM vê o erro ORIGINAL (mais simples) em vez do pior produzido pelo patch
- **N9 — Abort após 2 timeouts consecutivos por arquivo**: `ai/model.py` ganha contador module-level `_consecutive_timeouts`. `call_model` incrementa em `requests.exceptions.Timeout` e reseta em sucesso. `_try_local_agent` aborta antes da próxima attempt se `>= _TIMEOUT_ABORT_THRESHOLD=2`. `generate_tests` chama `reset_consecutive_timeouts()` no início de cada iteração `for main_file` (contador por-arquivo). APIs: `get_consecutive_timeouts()`, `reset_consecutive_timeouts()`. Reduz wall-clock máximo por arquivo difícil de ~35min para ~15min — corta cascata 14b×2 + 7b×2 = 4×420s antes de chegar nos 4 timeouts completos
- **N10 — Regra preventiva controller null-input**: `_build_active_rules` injeta bloco `### CONTROLLER NULL-INPUT TEST PATTERN` quando classe tem `@RestController` E método retorna `ResponseEntity`. Explica que `response.getBody()` NÃO é null por default — depende do que `service.method(null)` retornar. Oferece SOLUÇÃO 1 (`when(service.method(any())).thenReturn(null)`) e SOLUÇÃO 2 (assert na estrutura, não no body). Proíbe `assertNull(response.getBody())` sem stub. Coexiste com N4 (ResponseEntity vs @ResponseStatus) — ambas para controllers com ResponseEntity, cobrem gaps Spring distintos
